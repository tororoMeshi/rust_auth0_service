...
legacy uniauth JWT consumer として、`stateless-chat/nodejs-room`、`stateless-chat/websocket-chat-api`、`jamaica/play-matching`、`jamaica/matchmaking` が確認されている。各consumerは known legacy JWT consumer であり、過去decisionは `nodejs-room` = **MIGRATE**、`websocket-chat-api` = **RETIRE**、`play-matching` = **RETIRE**、`matchmaking` = **RETIRE** である。これらは migrated または retired の完了記録ではなく、Auth Foundation release prerequisite でもない。切替後の互換性は unsupported とする。external rollback は external owner の責務であり、cutover により external consumer が動作しなくなっても、それだけを理由に rust_auth0_service rollback を自動発動しない。rust_auth0_service rollback は external consumer を自動復元せず、同名 Secretだけを理由に他workloadへSecretを同期しない。
