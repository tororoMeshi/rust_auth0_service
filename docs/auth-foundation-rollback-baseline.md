...
legacy uniauth JWT consumer として、`stateless-chat/nodejs-room`、`stateless-chat/websocket-chat-api`、`jamaica/play-matching`、`jamaica/matchmaking` が確認されている。canonical decision は `nodejs-room` = **MIGRATE**、`websocket-chat-api` = **RETIRE**、`play-matching` = **RETIRE**、`matchmaking` = **RETIRE** であり、UNKNOWN は0件である。これらは external release prerequisite であり、rust_auth0_service は consumer name、decision、external ownership、required release condition、readiness/completion confirmation、legacy JWT shutdown go/no-go だけを保持する。external rollback は external owner の責務であり、rust_auth0_service rollback は retired external consumer を自動復元しない。同名 Secretだけを理由に他workloadへSecretを同期しない。
